...
namespace Bloom_Sample
{
    ...
    public class BloomFilter : IDisposable
    {
        ...
        public void Load(GraphicsDevice graphicsDevice, ContentManager content, int width, int height, SurfaceFormat renderTargetFormat = SurfaceFormat.Color, QuadRenderer quadRenderer = null)
        {
            ...
            _bloomEffect = content.Load<Effect>("Effects/Ambient");
        ...
        internal void SetBloomPreset(BloomPresets preset)
        {
            switch (preset)
            {
        ...
        public void UpdateResolution(int width, int height)
        {
            ...
            _bloomRenderTarget2DMip0 = new RenderTarget2D(_graphicsDevice,
                ...
                (int)Math.Max(1, height), false, _renderTargetFormat, DepthFormat.None, 0, RenderTargetUsage.DiscardContents);
            _bloomRenderTarget2DMip1 = new RenderTarget2D(_graphicsDevice,
                (int)Math.Max(1, width / 2),
                (int)Math.Max(1, height / 2), false, _renderTargetFormat, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
            _bloomRenderTarget2DMip2 = new RenderTarget2D(_graphicsDevice,
                (int)Math.Max(1, width / 4),
                (int)Math.Max(1, height / 4), false, _renderTargetFormat, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
            _bloomRenderTarget2DMip3 = new RenderTarget2D(_graphicsDevice,
                (int)Math.Max(1, width / 8),
                (int)Math.Max(1, height / 8), false, _renderTargetFormat, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
            _bloomRenderTarget2DMip4 = new RenderTarget2D(_graphicsDevice,
                (int)Math.Max(1, width / 16),
                (int)Math.Max(1, height / 16), false, _renderTargetFormat, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
            _bloomRenderTarget2DMip5 = new RenderTarget2D(_graphicsDevice,
                (int)Math.Max(1, width / 32),
                (int)Math.Max(1,height / 32), false, _renderTargetFormat, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
        }
